...
def load_packet_vendors_from_config(
    config_file_path: str,
) -> Dict[PacketType, List[Vendor]]:
    with open(config_file_path, "r") as f:
        packet_type_data = safe_load(f)

    packet_vendors = dict()
    for packet_name, allowed_vendors in packet_type_data.items():
        packet_vendors[PacketType.from_value(packet_name)] = [
            Vendor.from_value(vendor) for vendor in allowed_vendors
        ]
